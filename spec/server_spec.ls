...
describe "server" (_) ->
  ...
  describe "template assignment" (_) ->
    ...
    before-each !->
      ...
      app := start: jasmine.create-spy('spy').and.return-value ['app-state', 'body']
      inst := server app: app

    it "calls the method that renders a route to string" !->
      inst.render app, 'url', paths: { layouts: support-templates, public: 'dist' }
      # Test that the method that renders a route to a string has been called
      # with a URL an anonymous function.
      expect app.start .to-have-been-called-with 'url'

    it "throws an error if the specified template cannot be found" (done) !->

      var failed

      inst.render app, 'url', paths: { layouts: 'non-existant/path', public: 'dist' }
      .then (output) !->
        failed := false
      .catch (e) ->
        failed := true
    ...
    it "interpolates the instantiation partial with the author defined template" (done) !->
      inst.render app, 'app-state', paths: { layouts: support-templates, public: 'dist' }
        .then (template) !->
