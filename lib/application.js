(function(){
  var bluebird, cursor, dom, routes, serverRendering, cookie, domUtils, unesc, ref$, keys, each, Obj, map, reject, span, appComponent, initAppState, observePageChange;
  ...
  initAppState = function(appState, initialState, routeContext, cookies){
    appState == null && (appState = {});
    return cursor((appState.state = initialState, appState.route = routeContext, appState.cookies = cookies, appState));
  };
    ...
    create: function(app){
      return {
        start: function(){
          ...
          if (stateNode) {
            serverState = JSON.parse(unesc(stateNode.text));
          }
          appState = serverState
            ? cursor(serverState)
            : initAppState({}, app.getInitialState(), {}, {});
          app.start(appState);
          if (!serverState) {
            appState.get('cookies').update(function(){
              var clientCookies;
              clientCookies = cookie.parse(document.cookie);
        ...
        render: function(req, res){
          ...
          keys(
          clientCookies));
          appState = initAppState(req.appState, app.getInitialState(), null, []);
        ...
        processForm: function(req, res){
          var rootElement, path, clientCookies, parsedCookies, routeSet, appState, transaction;
          path = req.originalUrl;
          clientCookies = cookie.parse(req.headers.cookie || "");
          parsedCookies = map(function(k){
            return cookie.serialize(k, clientCookies[k]);
          ...
          return appState.endTransaction(transaction).then(function(){
            var formProcessingTransaction;
            formProcessingTransaction = appState.startTransaction();
            rootElement = appComponent({
              appState: appState,
              routes: routeSet
            });
            serverRendering.processForm(rootElement, appState, req.body, path);
            return appState.endTransaction(formProcessingTransaction);
          }).then(function(){
            var meta, body, location;
            meta = serverRendering.routeMetadata(rootElement, appState);
            body = !((location = serverRendering.getRedirect()) && location !== path) ? React.renderToString(rootElement) : null;
