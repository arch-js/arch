(function(){
  var Immutable, bluebird, ref$, map, take, reverse, reject, each, join, split, isType, empty, UpdateTransaction, isPromise, arrayCursor, objectCursor, notifyListeners, flushUpdates, performUpdate, Cursor, toString$ = {}.toString;
  Immutable = require('immutable');
  bluebird = require('bluebird');
  ref$ = require('prelude-ls'), map = ref$.map, take = ref$.take, reverse = ref$.reverse, reject = ref$.reject, each = ref$.each, join = ref$.join, split = ref$.split, isType = ref$.isType, empty = ref$.empty;
  ...
  arrayCursor = function(root, data, len, path){
    ...
    array._transactions = [];
    array._updates = [];
    ref$ = Cursor.prototype, array.get = ref$.get, array.deref = ref$.deref, array.raw = ref$.raw, array.update = ref$.update, array.isEmpty = ref$.isEmpty, array._swap = ref$._swap, array.onChange = ref$.onChange;
    return array;
  };
  ...
  performUpdate = function(cursor, update, forceUpdate){
  ...
  Cursor.prototype.update = function(cbk, forceUpdate){
    var updates, ref$, cursor, update, results$ = [];
    updates = this._root._updates;
    updates.push([this, cbk, forceUpdate]);
    if (!(updates.length < 2)) {
      return;
    }
    while (!empty(updates)) {
      ref$ = updates[0], cursor = ref$[0], update = ref$[1], forceUpdate = ref$[2];
      performUpdate(cursor, update, forceUpdate);
      results$.push(updates.shift());
    }
    return results$;
  };
  Cursor.prototype.forceUpdate = function(cbk){
    return this.update(cbk, true);
  };
  ...
  Cursor.prototype.removeListener = function(fn){
    var key;
    key = join('.', this._path);
    this._root._listeners[key] = reject((function(it){
      return it === fn;
    }))(
    this._root._listeners[key]);
  };
