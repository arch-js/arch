(function(){
  var express, fs, path, jade, bluebird, bodyParser, bundler, livescript, cookieParser, register, ref$, each, values, filter, find, flatten, map, first, defaults, archGet, archPost, __template, layoutRender;
  ...
  module.exports = function(options){
    ...
    server = express().use("/" + options.publicPath, express['static'](path.join(options.appPath, options.publicPath))).use(bodyParser.urlencoded({
      extended: false
    })).use(cookieParser());
