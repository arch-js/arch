(function(){
  var express, fs, path, jade, bluebird, bodyParser, bundler, livescript, register, xssFilters, ref$, each, values, filter, find, flatten, map, first, defaults, archGet, archPost, __template, layoutRender;
  ...
  layoutRender = function(meta, body, appState, options){
    var stringifyFilter, bundlePath, archBody, layout, title;
    stringifyFilter = function(k, v){
      if (typeof v === 'string') {
    ...
    archBody = __template({
      ...
      bundle: bundlePath,
      body: body,
      state: JSON.stringify(appState, stringifyFilter)
    });
