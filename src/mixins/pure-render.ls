...
module.exports =
  should-component-update: (next-props, next-state) ->
    ...
    for key, prop of @props
      return true unless key is 'children' or next-props[key] and prop.eq and prop.eq next-props[key]
    for key, state of @state
      return true unless next-state[key] and state.eq and state.eq next-state[key]
    false